...
function saveVersionFile(targetDir, version) {
    fs.writeFileSync(
        path.join(targetDir, VERSION_FILENAME),
        `${version}\n`,
        "utf8",
    );
}
...
const ENGINE_DETECTORS = {
    ...
    cline: (dir) =>
        fs.existsSync(path.join(dir, ".clinerules")) ||
        fs.existsSync(path.join(dir, ".cline")),
    roocode: (dir) =>
        fs.existsSync(path.join(dir, ".roorules")) ||
        fs.existsSync(path.join(dir, ".roo")),
...
function detectInstalledEngines(targetDir) {
    return Object.keys(ENGINE_DETECTORS).filter((id) =>
        ENGINE_DETECTORS[id](targetDir),
    );
}
...
function findPythonCommand() {
    ...
    for (const candidate of candidates) {
        ...
        if (result.status === 0 && output.trim().startsWith("Python"))
            return candidate;
    }
    return null;
}
...
function listFilesRecursive(dir, baseDir) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            ...
            results.push(
                path.relative(baseDir, absPath).split(path.sep).join("/"),
            );
        }
    }
    return results;
...
function agentRelFiles(baseDir) {
    return listFilesRecursive(path.join(baseDir, ".agent")).map((p) =>
        path.posix.join(".agent", p),
    );
}
...
function copyMcpConfigIfAbsent(targetDir) {
    ...
    console.log(
        `✔ Added starter .mcp.json (GitHub MCP server, OAuth — no token needed in the file)`,
    );
}
...
function promptIdeTarget(detectedEngines) {
    ...
    const defaultChoice =
        detectedEngines.length === 1 ? detectedEngines[0] : "skip";
    const detectedHint =
        detectedEngines.length > 0
            ? ` (detected: ${detectedEngines.join(", ")})`
            : "";
...
async function init(args) {
    ...
    const doctorOk = runPythonScript(
        pythonCmd,
        targetDir,
        ".agent/scripts/doctor.py",
        [],
    );
    if (!doctorOk) {
        console.error(
    ...
    if (fs.existsSync(gitDir)) {
        runPythonScript(
            pythonCmd,
            targetDir,
            ".agent/scripts/install_hooks.py",
            [],
        );
    } else {
        console.log(
            "ℹ No .git/ found — skipping pre-commit hook install. Run `git init` first if you want it.",
        );
    }
    ...
    if (explicitTargetArg) {
        ({ valid: ideTargets, invalid: invalidTargets } = parseIdeTargets(
            explicitTargetArg.split("=")[1] || "",
        ));
    } else if (process.stdin.isTTY) {
        ({ valid: ideTargets, invalid: invalidTargets } =
            await promptIdeTarget(detectedEngines));
    } else if (detectedEngines.length === 1) {
        ideTargets = [detectedEngines[0]];
        console.log(
            `ℹ Detected ${ideTargets[0]} in this project — syncing automatically (non-interactive mode).`,
        );
    }

    if (invalidTargets.length > 0) {
        console.log(
            `⚠ Ignoring unrecognized IDE name(s): ${invalidTargets.join(", ")}`,
        );
    }
    ...
    if (ideTargets.length > 0) {
        ...
        console.log(
            "ℹ Skipped IDE sync. Run `python .agent/scripts/sync_ide.py --target <ide>` whenever you're ready.",
        );
    }
...
function update(args) {
    ...
    console.log(`\n🏛️  DevBureau — Update${dryRun ? " [DRY-RUN]" : ""}\n`);

    if (!fs.existsSync(destAgentDir)) {
        console.error(
            "✘ No .agent/ found in this directory. Run `npx devbureau init` first.",
        );
        process.exitCode = 1;
        return;
    }
    ...
    // Skipped entirely in --dry-run: nothing below writes to disk, so there's
    // nothing to roll back.
    let backupDir = null;
    if (!dryRun) {
        backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "devbureau-backup-"));
    ...
    try {
        for (const relPath of sourceRelFiles) {
            ...
            const baselineHash = previousManifest
                ? previousManifest[relPath]
                : null;
            const isCustomized =
                !force && baselineHash && baselineHash !== destHash;
            ...
            if (!dryRun) fs.copyFileSync(sourceAbs, destAbs);
            newManifest[relPath] = sourceHash;
            updated++;
        }
    } catch (err) {
        ...
        if (backupDir) {
            fs.cpSync(backupDir, destAgentDir, {
                recursive: true,
                force: true,
            });
    ...
    if (customizedFiles.length > 0) {
        ...
        if (customizedFiles.length > 20)
            console.log(`    ... e mais ${customizedFiles.length - 20}`);
        console.log(
            "  Revise manualmente se quiser incorporar as melhorias do kit nesses arquivos.",
        );
        console.log(
            "  (Re-rode com --force se quiser sobrescrever mesmo assim — perde as suas edições.)",
        );
    }
    ...
    console.log(
        dryRun
            ? "\nℹ Dry run only — nothing was written. Re-run without --dry-run to apply.\n"
...
function uninstall(args) {
    ...
    if (!fs.existsSync(destAgentDir)) {
        console.error(
            "✘ No .agent/ found in this directory — nothing to uninstall.",
        );
        process.exitCode = 1;
        return;
    }
    ...
    if (!manifest) {
        ...
        console.log(
            "  Remove .agent/ manually if you're sure, or run `update` first to rebuild a manifest.",
        );
        return;
    }
    ...
    console.log(
        `✔ ${verb}: ${ideFilesRemoved} generated IDE file(s)/folder(s)`,
    );
    if (customized.length > 0) {
        console.log(`⚠ Left in place (customized, ${customized.length}):`);
        for (const f of customized.slice(0, 20)) console.log(`    - ${f}`);
        if (customized.length > 20)
            console.log(`    ... and ${customized.length - 20} more`);
        console.log(
            "  (Re-run with --force on `update` first if you want these overwritten, then uninstall.)",
        );
    }
    console.log(
...
Usage:
  ...
  npx devbureau update [--force] [--dry-run]
...
Commands:
  ...
  update      Pull the latest .agent/ from the installed DevBureau version into
              ...
              --dry-run reports what would change (including the changelog
              since your installed version) and writes nothing — run this
              anytime to check if you're behind without touching any files.

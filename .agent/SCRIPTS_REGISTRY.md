...
| `memory_recall.py` (dead ends) | Recall também cobre `dead-ends.md`, o registro automático de abordagens que já falharam 2 ou 3 vezes seguidas | "already tried this", "dead end" / "já tentei isso", "beco sem saída" |
| `memory_rotate.py` | Rotates dated entries out of memory files above 50 KB into `archive/`, refreshing `archive/INDEX.md`; `--check` reports, `--dry-run` previews. Nothing is deleted and recall still finds archived entries | "rotate memory", "memory file too big" / "rotacionar memória", "arquivo de memória grande" |
| `context_cost.py` | `record` (SessionEnd) attributes tool-output size to the files each call targeted; `report` ranks recurring context hogs across sessions | "context cost", "what is eating my context", "biggest files read" / "custo de contexto", "o que está consumindo contexto" |
...
| `memory_recall.py` | Searches `lessons.md`/`gotchas.md` by trigger keyword (`recall`); lists entries never recalled and N+ days old (`stale`); records a recall timestamp on an entry (`mark`) | "recall a lesson", "have we seen this before", "stale memory" / "já vimos isso antes", "buscar lição", "memória parada" |
| `test_gap_check.py` | Flags changed source files with no matching test file touched in the same diff (naming-convention match, not a coverage tool); `--diff`, `--strict`, `--json` | "test gap", "did I forget a test" / "lacuna de teste", "esqueci de testar" |
| `doc_drift_check.py` | Flags backtick-quoted file/script references in the kit's core docs that no longer exist on disk; `--files`, `--strict`, `--json` | "doc drift", "dead reference", "broken doc link" / "documentação desatualizada", "referência morta" |
| `integrity_manifest.py` | Generates (`generate`) or verifies (`verify`) a SHA-256 manifest of `.agent/rules/`, `.agent/agents/`, `.agent/workflows/` — `generate` before a release, `verify` in a derived project to detect drift from the shipped baseline | "verify kit integrity", "did the rules change" / "verificar integridade do kit", "as regras mudaram?" |

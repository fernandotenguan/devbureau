...
| `.agent/scripts/hooks/detect_tool_loop.py` | Claude Code only (`PostToolUse`) | Edit/Write/MultiEdit/Bash/Grep/Read | Advisory-only: mirrors 2 rows of DEVBUREAU.md's Loop Detection Rules table (same tool+args+error 3×, Edit content-mismatch 2×) using per-session state in `_hook_state.py`; when a rule fires it also appends the failed approach to `.agent/memory/dead-ends.md` via `_dead_ends.py` (secrets redacted, one entry per signature) so the dead end survives the session — never blocks |
...
| `.agent/scripts/hooks/protect_tests.py` | Claude Code only (`PreToolUse`) | Bash `rm`/`git rm` on a test path, or Edit/Write on a test file that adds a skip/xfail marker or shrinks it below 40% of its size | Blocks it, making the Zero-Break rule "never fake a green suite" hard-blocking. Creating new tests is always allowed. Escape: `DEVBUREAU_ALLOW_TEST_EDITS=1` |

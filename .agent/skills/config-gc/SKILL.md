...
| Stale memory entries | `python .agent/scripts/memory_recall.py stale` for individual `lessons.md`/`gotchas.md` entries never recalled; compare `.agent/memory/*.md` files against references in `pattern-mining-log.md`'s merge-status table and this session's own auto-memory index for whole files nothing links to | An individual entry the script flags, or a memory file nothing else links to or supersedes |
